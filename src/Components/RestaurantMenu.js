...
import useRestaurant from "../useHook/useRestaurant";
    ...
    <div className="flex flex-col w-[80%] md:w-2/3 p-4 border m-auto">
      <div className="flex flex-col justify-between pb-4 border-b md:flex-row gap-3">
        ...
        <img
          ...
          src={
            IMG_CDN_URL + resMenu?.cards[0]?.card?.card?.info?.cloudinaryImageId
          }
          alt=""
        />
      </div>
      ...
      <h1 className="font-bold mt-4 mb-10">
        Recommended (
        {
          resMenu?.cards[resMenu?.cards.length - 1]?.groupedCard?.cardGroupMap
            ?.REGULAR?.cards[2]?.card?.card?.itemCards?.length
        }
        )
      </h1>
      <div className="">
        ...
        ):"No servings right now!! Sorry for the inconvenience"}
    </div>
    </div >
  );
};
